...
export function createPolicyCard(p, sym, TODAY, CURRENT_YEAR) {
    ...
    const matY = parseInt((p.maturity || "01 Jan 2000").split(' ')[2]);
    const premEndYear = parseInt((p.premiumEnds || "01 Jan 2000").split(' ')[2]);
    ...
    <div class="policy-card mb-6" id="card-${p.id}" style="border-left: 16px solid ${brandColor}; border-color: ${brandColor};">
        <div class="card-header transition-colors" style="background: ${brandBg};" onclick="toggleCard('${p.id}')">
            ...
            <div class="flex gap-12 items-center mr-6">
                <div class="flex items-center w-[260px] -ml-4">
                    <div class="funky-badge" style="border-color: ${brandColor}; box-shadow: 0 0 10px ${brandColor}44;">${p.type || 'SAVINGS'}</div>
                    <div class="ml-6">
        ...
        <div class="content-area" style="background: linear-gradient(to bottom, ${brandBg}, #ffffff)">
            ...
            <div class="timeline-track">
                <div class="absolute -top-8 left-0 text-[11px] font-black text-slate-400 uppercase">${p.commenced || 'N/A'}</div>
                ${timelineHtml}
                <div class="absolute -top-8 right-0 text-[11px] font-black text-slate-400 uppercase">${p.maturity || 'N/A'}</div>
            </div>
        </div>
    </div>`;
}
