/* component_sg.js - Baseline v3.8.5 (Fixed Hover & Clean Header) */
...
export function createSGCard(p, sym, TODAY, CURRENT_YEAR) {
    ...
    for(let polY = 1; polY <= totalSegments; polY++) {
        ...
        // Fixed the hover tooltip to show Defined Text instead of "?"
        timelineHtml += `
            <div class="segment ${color} relative group h-8 flex-1 border-r border-white/5 first:rounded-l-lg transition-all">
                <div class="opacity-0 group-hover:opacity-100 absolute bottom-full mb-2 left-1/2 -translate-x-1/2 bg-slate-900 text-white px-2 py-1 rounded text-[9px] z-50 whitespace-nowrap shadow-xl pointer-events-none transition-opacity duration-200">
                    <b class="text-sky-400 uppercase tracking-tighter">YR ${polY} (${yr})</b><br>
                    ${isCurrent ? 'Current' : (chargeAtYear > 0 ? 'Locked' : 'Vested')}
                </div>
            </div>`;
    }

    // --- STAR LOGIC (Clean Hover / No Truncation) ---
    const starHtml = `
        <div class="mat-star relative group flex items-center justify-center px-4 h-8 border-l border-slate-300 ml-1">
            ...
            <div class="opacity-0 group-hover:opacity-100 absolute bottom-full right-0 mb-3 bg-slate-900 text-white p-3 rounded-xl z-50 shadow-2xl border border-white/10 pointer-events-none transition-all duration-200">
    ...
    <div class="policy-card mb-8 rounded-[32px] bg-white shadow-xl border-t-8 border-red-600 overflow-hidden" id="card-${p.id}">
        <div class="card-header p-8 flex items-center justify-between cursor-pointer hover:bg-slate-50 transition-colors" onclick="toggleCard('${p.id}')">
        ...
        <div class="content-area px-8 pb-10 pt-4 bg-slate-50 border-t">
            <div class="grid grid-cols-3 gap-4 mb-10">
                <div class="bg-slate-900 p-5 rounded-[24px] text-center"><p class="text-[9px] text-slate-500 uppercase font-bold">Policy Number</p><p class="text-sm font-black text-lime-400 font-mono tracking-wider">${p.id}</p></div>
                <div class="bg-emerald-600 p-5 rounded-[24px] text-center shadow-lg"><p class="text-[9px] text-emerald-100 uppercase font-bold">Surrender Value</p><p class="text-2xl font-black text-white">${autoFmt(surrenderValue, sym)}</p></div>
                <div class="bg-white p-5 rounded-[24px] border border-red-100 text-center"><p class="text-[9px] text-red-400 uppercase font-bold">Locked Value</p><p class="text-2xl font-black text-red-600">-${autoFmt(lockedValue, sym)}</p></div>
            </div>

            <div class="flex justify-between items-end mb-2 px-1 text-[11px] font-black text-slate-400 uppercase tracking-widest">
