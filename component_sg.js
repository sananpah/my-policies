/* component_sg.js - Baseline v3.6.6 (Final Layout Alignment) */
...
export function createSGCard(p, sym, TODAY, CURRENT_YEAR) {
    ...
    for(let polY = 1; polY <= totalSegments; polY++) {
        ...
        timelineHtml += `
            <div class="segment ${color} relative group h-10 flex-1 border-r border-white/10 transition-all first:rounded-l-lg last:rounded-r-lg">
                ${isCurrent ? '<div class="absolute inset-0 bg-white/20 animate-pulse"></div>' : ''}
                <div class="opacity-0 group-hover:opacity-100 absolute bottom-full mb-2 left-1/2 -translate-x-1/2 bg-slate-900 text-white p-2 rounded text-[9px] z-50 whitespace-nowrap shadow-xl pointer-events-none">
        ...
        <div class="content-area px-8 pb-10 pt-6 bg-slate-50 border-t">
            ...
            <div class="flex justify-between items-end mb-2 px-1">
                <div class="flex flex-col">
                    <span class="text-[9px] font-bold text-slate-400 uppercase">Commenced</span>
                    <span class="text-xs font-black text-slate-700">${p.commenced}</span>
                </div>
                <div class="text-right leading-none pb-1">
                    <div class="text-[10px] font-black text-slate-500 italic transform -skew-x-12 tracking-widest">⭐ MATURITY</div>
                    <div class="text-[8px] font-bold text-slate-400 italic transform -skew-x-12 tracking-widest mt-1 uppercase">Unit Value</div>
                </div>
                <div class="flex flex-col text-right">
                    <span class="text-[9px] font-bold text-slate-400 uppercase">Maturity Date</span>
                    <span class="text-xs font-black text-slate-700">${p.maturity}</span>
                </div>
            </div>

            <div class="flex items-center gap-3">
                <div class="timeline-track flex h-10 bg-slate-200 rounded-xl overflow-visible p-1 shadow-inner flex-1">
                    ${timelineHtml}
                </div>
                
                <div class="relative group cursor-help flex-shrink-0">
                    <div class="w-10 h-10 flex items-center justify-center bg-slate-900 rounded-xl shadow-lg border-b-4 border-amber-400 transition-all group-hover:scale-110">
                        <span class="text-amber-400 text-lg">⭐</span>
                    </div>
                    <div class="opacity-0 group-hover:opacity-100 absolute bottom-full mb-3 right-0 bg-slate-900 text-white p-2 rounded text-[9px] z-50 whitespace-nowrap shadow-2xl border border-amber-400/30 transition-opacity">
            ...
            <div class="flex gap-4 text-[9px] font-bold text-slate-400 uppercase tracking-tighter mt-4 px-1">
