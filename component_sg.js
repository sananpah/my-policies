/* component_sg.js - v6.2.2 - Full Build with Restored Tooltips */
    ...
    const capitalAnalysisHtml = p.totalPremiumPaid ? `
        <div class="mb-8 p-6 rounded-[32px] bg-white border border-slate-100 shadow-sm">
            ...
            <div class="grid grid-cols-3 gap-4">
                <div class="p-3 rounded-2xl bg-slate-50"><p class="text-[9px] font-bold text-slate-400 uppercase">Actual Paid</p><p class="font-black text-slate-700">${autoFmt(p.totalPremiumPaid, sym)}</p></div>
    ...
    <div class="policy-card mb-10 rounded-[40px] bg-white overflow-hidden shadow-[0_20px_50px_rgba(0,0,0,0.08)] border border-slate-100 relative" id="card-${p.id}">
        ...
        <div class="p-8 flex items-center justify-between cursor-pointer relative" style="background: ${brandBg}" onclick="toggleCard('${p.id}')">
            <div class="flex items-center gap-8 px-4">
                ...
                <div><h3 class="font-black text-3xl text-slate-900 mb-2">${p.name}</h3>
                <div class="flex items-center gap-3"><span class="px-3 py-1 rounded-md text-[10px] font-bold text-white uppercase" style="background: ${brandColor}">${p.company}</span><span class="font-mono text-xs font-bold text-slate-400">#${p.id}</span></div></div>
            </div>
        ...
        <div class="content-area px-10 pb-10 pt-2" style="background: linear-gradient(to bottom, ${brandBg}, #ffffff)">
            <div class="grid grid-cols-4 gap-6 mb-8">
                ...
                <div class="p-6 rounded-[32px] bg-emerald-50 border border-emerald-100"><p class="text-[10px] font-black text-emerald-600 mb-2 uppercase">Surrender</p><p class="text-3xl font-black text-emerald-700">${autoFmt(surrenderValue, sym)}</p></div>
                <div class="p-6 rounded-[32px] bg-red-50 border border-red-100"><p class="text-[10px] font-black text-red-400 mb-2 uppercase">Locked</p><p class="text-3xl font-black text-red-600">-${autoFmt(lockedValue, sym)}</p></div>
            </div>
            ${capitalAnalysisHtml}
