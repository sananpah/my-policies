/* component_sg.js - v6.8.8 - Design Sync: Border & Card Structure */
...
export function createSGCard(p, sym, TODAY, CURRENT_YEAR) {
    ...
    return `
    <div class="policy-card mb-6 overflow-hidden bg-white shadow-sm border border-slate-200 relative" id="card-${p.id}" style="border-left: 16px solid ${brandColor};">
        <div class="card-header p-4 flex items-center justify-between cursor-pointer relative transition-colors border-b border-slate-100" style="background: ${brandBg};" onclick="toggleCard('${p.id}')">
        ...
        <div class="content-area px-8 pb-8 pt-6 relative" style="background: linear-gradient(to bottom, ${brandBg}, #ffffff)">
            <div class="grid grid-cols-2 gap-4 mb-4">
                <div class="p-4 rounded-xl bg-slate-50/50 border border-slate-100 shadow-sm">
                    ...
                    <p class="text-lg font-bold text-slate-700 tracking-tight">#${p.id}</p>
                </div>
                <div class="p-4 rounded-xl bg-white border border-slate-100 shadow-sm">
                    ...
                    <p class="text-xl font-black text-slate-900 tracking-tighter">${autoFmt(accountValue, sym)}</p>
                </div>
            </div>
            ...
            <div class="grid grid-cols-3 gap-4 mb-6">
                <div class="p-4 rounded-xl bg-white border border-slate-100 shadow-sm">
                    ...
                    <p class="text-xl font-black text-slate-800 tracking-tighter">${autoFmt(netInvested, sym)}</p>
                ...
                <div class="p-4 rounded-xl bg-emerald-50/50 border border-emerald-100 text-center shadow-sm">
                    ...
                    <p class="text-2xl font-black text-emerald-700 tracking-tighter">${autoFmt(surrenderValue, sym)}</p>
                </div>
                <div class="p-4 rounded-xl bg-red-50/50 border border-red-100 text-center shadow-sm">
                    ...
                    <p class="text-2xl font-black text-red-600 tracking-tighter">-${autoFmt(accountValue - surrenderValue, sym)}</p>
                </div>
            </div>
