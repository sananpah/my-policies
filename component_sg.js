/* component_sg.js - v6.5.5 - Simplified Zero-Based MIP Logic */
...
export function createSGCard(p, sym, TODAY, CURRENT_YEAR) {
    ...
    // 1. DYNAMIC DATA (Simplified: 0 = Vested, >0 = Countdown)
    ...
    if (isPaidUp || mip === 0) {
        ...
        // Simple Logic: Target is always Commencement + MIP years
    ...
    // Timeline Scale
    let maxYears = Math.max(mip, 15, policyYearIdx + 10);
    ...
    for (let polY = 1; polY <= maxYears; polY++) {
        ...
        if (yr < CURRENT_YEAR) {
            ...
            if (polY <= mip && mip > 0) {
                colorClass = "bg-indigo-500"; statusLabel = "Strict Lock (MIP)";
            } else if (polY <= ppt) {
    ...
    // 3. REMAINING COMPONENT UI (Logos, Financials, etc.)
    ...
    <div class="policy-card mb-10 rounded-[40px] bg-white overflow-hidden shadow-[0_20px_50px_rgba(0,0,0,0.08)] border-2 relative" id="card-${p.id}" style="border-left: 16px solid ${brandColor}; border-color: ${brandColor};">
        <div class="p-8 flex items-center justify-between cursor-pointer relative" style="background: ${brandBg}" onclick="toggleCard('${p.id}')">
            ...
            <div class="flex items-center gap-10 text-right px-4 relative z-20">
                ...
                <div class="bg-white/60 px-6 py-3 rounded-[20px] border border-white/50 flex flex-col justify-center min-w-[125px] h-[64px]">
                    <p class="text-[9px] font-black text-indigo-500 uppercase text-center">${premRemainingStr === "Vested" ? 'Status' : 'Left: ' + premRemainingStr}</p>
                    ${premRemainingStr === "Vested" ? 
                        `<p class="text-sm font-black text-emerald-700 text-center uppercase tracking-tighter">FULLY VESTED</p>` : 
        ...
        <div class="content-area px-10 pb-10 pt-2 relative z-20" style="background: linear-gradient(to bottom, ${brandBg}, #ffffff)">
            <div class="grid grid-cols-5 gap-4 mb-8">
                ...
                <div class="p-6 rounded-[32px] bg-emerald-50 border border-emerald-100 shadow-sm"><p class="text-[10px] font-black text-emerald-600 mb-2 uppercase text-center">Surrender</p><p class="text-2xl font-black text-emerald-700 text-center">${autoFmt(Math.round(Math.max(0, accountValue - ((p.surrenderCharges && p.surrenderCharges[policyYearIdx]) || 0) / 100 * (isPaidUp ? accountValue : totalPremiumsPaid))), sym)}</p></div>
                <div class="p-6 rounded-[32px] bg-red-50 border border-red-100 shadow-sm"><p class="text-[10px] font-black text-red-400 mb-2 uppercase text-center">Locked</p><p class="text-2xl font-black text-red-600 text-center">-${autoFmt(accountValue - Math.round(Math.max(0, accountValue - ((p.surrenderCharges && p.surrenderCharges[policyYearIdx]) || 0) / 100 * (isPaidUp ? accountValue : totalPremiumsPaid))), sym)}</p></div>
            </div>
            ${capHtml}
