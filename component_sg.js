/* component_sg.js - v7.0.7 - Full Standardized Layout (Curvature + Attributes + Base Math) */
...
export function createSGCard(p, sym, TODAY, CURRENT_YEAR) {
    ...
    // --- DUAL-ENGINE SURRENDER MATH (Flag Aware) ---
    ...
    const timelineYearsToMat = endY - startY;
    let maxYears = (timelineYearsToMat <= 25) ? timelineYearsToMat : Math.min(Math.max(15, policyYearIdx + 5), timelineYearsToMat);
    let timelineHtml = '';
    ...
    // Original (Restored) Star Tooltip
    const starHtml = `
        <div class="ml-2 relative group flex items-center justify-center w-12 h-10 bg-white rounded-xl shadow-sm border border-slate-200 cursor-help">
            ...
            <div class="opacity-0 group-hover:opacity-100 absolute bottom-full mb-4 right-0 bg-slate-900 text-white p-4 rounded-2xl z-[100] shadow-2xl border border-white/10 pointer-events-none min-w-[180px]">
                ...
                <div class="space-y-1">
                    <div class="flex justify-between text-[10px] font-black leading-tight text-white"><span>Est. @4%:</span><span>${autoFmt(calcProj(0.04), sym)}</span></div>
                    <div class="flex justify-between text-[10px] font-black leading-tight text-white"><span>Est. @8%:</span><span>${autoFmt(calcProj(0.08), sym)}</span></div>
                </div>
    ...
    <div class="policy-card mb-10 rounded-[40px] bg-white overflow-hidden shadow-[0_20px_50px_rgba(0,0,0,0.08)] border-2 relative" 
            ...
            <div class="flex items-center gap-8 pl-6 pr-4">
                ...
                <div class="relative z-20">
                    ...
                    <div class="flex items-center gap-3">
                        <div class="funky-badge-v2" style="border-color:${brandColor}; color:${brandColor}; background:#fff; font-size:10px; font-weight:900; padding:2px 8px; border-radius:6px; border:1.5px solid; text-transform:uppercase;">
                            ${p.type || 'Savings'}
                        </div>
        ...
        <div class="content-area px-10 pb-10 pt-2 relative z-20" style="background: linear-gradient(to bottom, ${brandBg}, #ffffff)">
            <div class="grid grid-cols-4 gap-6 mb-8">
                ...
                <div class="p-6 rounded-[32px] bg-white border border-slate-100 shadow-sm"><p class="text-[10px] font-black text-slate-400 mb-2 uppercase">Invested (Net)</p><p class="text-2xl font-black text-slate-800" style="font-family:'Orbitron'">${autoFmt(netInvested, sym)}</p></div>
                <div class="p-6 rounded-[32px] bg-emerald-50 border border-emerald-100 shadow-sm text-center"><p class="text-[10px] font-black text-emerald-600 mb-2 uppercase">Surrender</p><p class="text-3xl font-black text-emerald-700" style="font-family:'Orbitron'">${autoFmt(surrenderValue, sym)}</p></div>
                <div class="p-6 rounded-[32px] bg-red-50 border border-red-100 shadow-sm text-center"><p class="text-[10px] font-black text-red-400 mb-2 uppercase">Locked</p><p class="text-3xl font-black text-red-600" style="font-family:'Orbitron'">${lockedDisplay}</p></div>
            </div>
            ...
            <div class="mb-6 p-4 bg-white/50 border border-slate-100 rounded-[24px] shadow-sm flex flex-col gap-2">
                ...
                <div class="flex items-center h-10">
                    ${(p.nomineeStatus === "NA") ? `<span class="text-[11px] font-black text-slate-400 uppercase italic">N/A</span>` :
