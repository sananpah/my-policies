/* component_sg.js - v6.6.4 - Compound Growth + Premium Projection (4%) */
...
export function createSGCard(p, sym, TODAY, CURRENT_YEAR) {
    ...
    // Growth of existing pot (Lump Sum FV)
    const fvUnits = accountValue * Math.pow(1 + r, yearsToMat);
    
    // Growth of future contributions (Annuity Due FV)
    let fvPremiums = 0;
    if (yearsToPay > 0) {
        // Compound future premiums
        fvPremiums = annualPremium * ((Math.pow(1 + r, yearsToPay) - 1) / r) * (1 + r);
        // If premiums stop before maturity, grow the accumulated sum to the end date
        const gapYears = yearsToMat - yearsToPay;
        if (gapYears > 0) fvPremiums = fvPremiums * Math.pow(1 + r, gapYears);
    }
    
    const totalProjected = Math.round(fvUnits + fvPremiums);

    // Timeline Progress Math
    let yearsPassed = CURRENT_YEAR - startY;
    if (TODAY < thisYearAnniversary) yearsPassed--;
    const policyYearIdx = yearsPassed + 1;
    let maxYears = Math.min(40, Math.max(mip, 15, policyYearIdx + 5, endY - startY));
    ...
    const starHtml = `
        <div class="ml-2 relative group flex items-center justify-center w-12 h-10 bg-white rounded-xl shadow-sm border border-slate-200 cursor-help">
            ...
            <div class="opacity-0 group-hover:opacity-100 absolute bottom-full mb-4 right-0 bg-slate-900 text-white p-4 rounded-2xl z-[100] shadow-2xl border border-white/10 pointer-events-none min-w-[240px]">
                <b class="text-amber-400 uppercase tracking-widest block text-[9px] mb-2 font-black">Maturity Projection (4%)</b>
                <div class="space-y-1">
                    <div class="flex justify-between text-[10px] text-slate-400"><span>Current Units:</span><span class="text-white font-bold">${p.currentUnitValue}</span></div>
                    <div class="flex justify-between text-[10px] text-slate-400"><span>Remaining Payments:</span><span class="text-white font-bold">${autoFmt(yearsToPay * annualPremium, sym)}</span></div>
                    <div class="h-[1px] bg-white/10 my-1"></div>
                    <div class="flex justify-between text-[13px] text-emerald-400 font-black"><span>Est. Value:</span><span>${autoFmt(totalProjected, sym)}*</span></div>
                </div>
                <div class="mt-2 pt-2 border-t border-white/10 text-[8px] text-slate-400 italic font-medium leading-tight">
                    *Based on current valuation + ${yearsToPay} future premiums compounded @ 4% till ${endY}. Non-guaranteed.
                </div>
        ...
        <div class="p-8 flex items-center justify-between cursor-pointer relative" style="background: ${brandBg}" onclick="toggleCard('${p.id}')">
            ...
            <div class="flex items-center gap-10 text-right px-4 relative z-20">
                ...
                <div class="bg-white/60 px-6 py-3 rounded-[20px] border border-white/50 flex flex-col justify-center min-w-[125px] h-[64px]">
                    <p class="text-[9px] font-black ${vestingStr === "Vested" ? 'text-emerald-500' : 'text-indigo-500'} uppercase text-center">${vestingStr === "Vested" ? 'Vested' : 'Left: ' + vestingStr}</p>
                    <div class="h-[1px] bg-slate-200/40 w-full my-1"></div>
                    ${isPaidUp ? `<p class="text-[9px] font-black text-emerald-600 uppercase text-center">Fully Paid</p>` : `<p class="text-[9px] font-black text-sky-500 uppercase text-center">Due: <span class="text-slate-700">${nextDueDisplay}</span></p>`}
                </div>
            </div>
        </div>
